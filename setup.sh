...
# Add this function before _run_setup_menu() function
...
_show_help_panel() {
  ...
  if ((REMOTE_MODE)); then
    ...
    echo -e "${BARR}  ${RST}      ${DIM}Git URL to clone ProjectR from${RST}" >&2
    echo -e "${BARR}  ${RST}      ${DIM}Useful for forks or private mirrors${RST}" >&2
  ...
  echo -e "${BARR}  ├──────────────────────────────────────────────────────────────────┤${RST}" >&2
  echo -e "${BARR}  │${RST}  • ${OPTION}Environment variables${RST} can override defaults:                  ${BARR}│${RST}" >&2
  echo -e "${BARR}  │${RST}    ${BRIGHT_BLACK}PROJECTR_COMMAND_NAME, PROJECTR_INSTALL_DIR, PROJECTR_BIN_DIR ${BARR}│${RST}" >&2
  echo -e "${BARR}  │${RST}    ${BRIGHT_BLACK}PROJECTR_REPO_URL                                             ${BARR}│${RST}" >&2
  echo -e "${BARR}  │${RST}  • ${OPTION}Non-interactive install${RST}: ${DIM}bash setup.sh --no-menu${RST}              ${BARR}│${RST}" >&2
  echo -e "${BARR}  │${RST}  • ${OPTION}After install${RST}, run ${BOLD_WHITE}${COMMAND_NAME} --help${RST} for command usage           ${BARR}│${RST}" >&2
...
maybe_add_path() {
  ...
  for rc in "$HOME/.bashrc" "$HOME/.zshrc" "$HOME/.profile"; do
    ...
    touch "$rc" || {
      warn "Could not update shell rc file: $rc"
      continue
    }
