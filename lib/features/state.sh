...
projectr_state_record_install() {
    ...
    if [[ $# -eq 4 ]]; then
        tool_id="$1"; name="$1"; package="$2"; manager="$3"; install_type="pkg"; cmd="$4"
    else
        tool_id="$1"; name="$2"; package="$3"; manager="$4"; install_type="$5"; cmd="$6"
    fi
...
projectr_state_record_action() {
    ...
    if command -v sqlite3 >/dev/null 2>&1; then
        ...
        mv "$tmp" "$PROJECTR_ACTIONS_TSV" || { rm -f "$tmp"; return 1; }
    fi
}
