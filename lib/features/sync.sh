...
pkg_update() {
  ...
  while ((attempt <= max_attempts)); do
    if ((attempt > 1)); then
      msg_warning "Attempt $attempt of $max_attempts failed. Retrying in $((attempt * 2)) seconds..."
      sleep $((attempt * 2))
    fi
...
pkg_upgrade() {
  ...
  snap)
    ...
    ;;
  winget)
    winget upgrade --all --silent >/dev/null 2>&1
    exit_code=$?
    ;;
  *)
    echo -e "${ERROR}"
  ...
  if [[ $exit_code -ne 0 ]]; then
    ...
    echo -e "${BOLD_GREEN}  [*] System upgrade completed successfully via $PM (exit code: $exit_code).${RST}"
  fi
