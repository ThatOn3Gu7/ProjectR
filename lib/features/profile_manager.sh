...
import_profile() {
    ...
    [[ -z "$file" ]] && { echo -e "${ERROR} [✗] No profile file specified for import.${RST}"; return 1; }
    [[ ! -f "$file" ]] && { echo -e "${ERROR} [✗] Profile configuration file not found: $file${RST}"; return 1; }
    [[ ! -r "$file" ]] && { echo -e "${ERROR} [✗] Profile configuration file is not readable: $file${RST}"; return 1; }
