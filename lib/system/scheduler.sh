...
projectr_updates_available() {
    ...
    case "$pm" in
        ...
        dnf|yum) "$pm" check-update >/dev/null 2>&1; [[ $? -eq 100 ]] ;;
...
EOF
    ...
    if command -v crontab >/dev/null 2>&1; then
        local tmp
        tmp=$(mktemp) || return 1
        { crontab -l 2>/dev/null | grep -Fv "$PROJECTR_SCHEDULER_CRON_TAG"; projectr_scheduler_cron_line; } > "$tmp"
        crontab "$tmp"
        rm -f "$tmp"
        echo "cron enabled"
        return 0
...
projectr_scheduler_cli() {
    ...
    case "$action" in
        ...
        *) echo "Unknown scheduler action: $action" >&2; return 1 ;;
    esac
}
