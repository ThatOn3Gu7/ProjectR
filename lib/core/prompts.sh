...
'
  ...
  while [ "$#" -gt 0 ]; do
    ...
    -p | --prompt)
      [ "$#" -ge 2 ] || {
        ask__err "$_arg requires a value"
        return 2
      }
      prompt=$2
      shift 2
    ...
    -d | --default)
      ...
      default_choice=$(ask__lower "$2")
      shift 2
      ;;
    --default=*)
      default_choice=$(ask__lower "${_arg#*=}")
      shift
      ;;
    -t | --timeout)
      [ "$#" -ge 2 ] || {
    ...
    --selected)
      [ "$#" -ge 2 ] || {
        ask__err "--selected requires a value"
        return 2
      }
      selected=$(ask__lower "$2")
      shift 2
      ;;
    --selected=*)
      selected=$(ask__lower "${_arg#*=}")
      shift
      ;;
    --non-interactive)
      [ "$#" -ge 2 ] || {
        ask__err "--non-interactive requires a value"
        return 2
      }
      non_interactive=$(ask__lower "$2")
      shift 2
      ;;
    --non-interactive=*)
      non_interactive=$(ask__lower "${_arg#*=}")
      shift
      ;;
    --style)
      [ "$#" -ge 2 ] || {
        ask__err "--style requires a value"
        return 2
      }
      style=$(ask__lower "$2")
      shift 2
      ;;
    --style=*)
      style=$(ask__lower "${_arg#*=}")
      shift
      ;;
    --align)
      [ "$#" -ge 2 ] || {
        ask__err "--align requires a value"
        return 2
      }
      align=$(ask__lower "$2")
      shift 2
      ;;
    --align=*)
      align=$(ask__lower "${_arg#*=}")
      shift
      ;;
    --color)
      [ "$#" -ge 2 ] || {
        ask__err "--color requires a value"
        return 2
      }
      color=$(ask__lower "$2")
      shift 2
    ...
    --icon)
      [ "$#" -ge 2 ] || {
        ask__err "--icon requires a value"
        return 2
      }
      icon=$2
      icon_custom=1
    ...
    --yes-icon)
      [ "$#" -ge 2 ] || {
        ask__err "--yes-icon requires a value"
        return 2
      }
      yes_icon=$2
      yes_icon_custom=1
    ...
    --pointer)
      [ "$#" -ge 2 ] || {
        ask__err "--pointer requires a value"
        return 2
      }
      pointer=$2
      pointer_custom=1
  ...
  # Validate config
  # -----------------------------
  case $default_choice in yes | y | true | 1) default_choice=yes ;; no | n | false | 0 | cancel) default_choice=no ;; none | '') default_choice="" ;; *)
    ask__err "--default must be yes, no, or none"
    return 2
    ;;
  esac
  case $selected in yes | y | true | 1) selected=yes ;; no | n | false | 0 | cancel) selected=no ;; *)
    ask__err "--selected must be yes or no"
    return 2
    ;;
  esac
  [ -n "$default_choice" ] && selected=$default_choice
  case $non_interactive in default | yes | no | fail) : ;; *)
    ask__err "--non-interactive must be default, yes, no, or fail"
    return 2
    ;;
  esac
  case $style in default | auto | classic) style=classic ;; plain | minimal | fancy | modern | neon | danger | success) : ;; *)
    ask__err "--style must be one of: classic, plain, minimal, fancy, modern, neon, danger, success"
    return 2
    ;;
  esac
  case $align in left | center | right) : ;; *)
    ask__err "--align must be left, center, or right"
    return 2
    ;;
  esac
  case $color in auto | always | never) : ;; *)
    ask__err "--color must be auto, always, or never"
    return 2
    ;;
  esac
  if [ -n "$timeout" ]; then
    ask__is_int "$timeout" || {
      ask__err "--timeout must be a positive integer"
      return 2
    }
    [ -n "$default_choice" ] || {
      ask__err "--timeout requires --default yes or --default no"
      return 2
    }
  fi
  ...
  danger)
    ...
    [ "$pointer_custom" -eq 1 ] || pointer="»"
    C_ICON="$C_RED"
    C_PROMPT="$C_BOLD$C_RED"
    C_YES="$C_RED"
    C_NO="$C_GREEN"
    C_HINT="$C_DIM$C_YELLOW"
    C_SELECTED="$C_REV$C_BOLD"
    ;;
  success)
    [ "$icon_custom" -eq 1 ] || icon="✓"
  ...
  if ! [ -t 0 ] || ! [ -t 2 ]; then
    ...
    no)
      ...
      ;;
    fail)
      [ "$quiet" -eq 1 ] || ask__err "not running in an interactive terminal"
      return 2
      ;;
    default)
  ...
  ask__render() {
    ...
    if [ "$border" -eq 1 ]; then
      ...
      len3=${#plain3}
      maxw=$len1
      [ "$len2" -gt "$maxw" ] && maxw=$len2
      [ "$len3" -gt "$maxw" ] && maxw=$len3
      inner=$((maxw + 2))
      local prefix
      ...
      render_mode=compact
      render_maxw=$compact_len
      render_prefix=$compact_prefix
