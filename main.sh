...
EOF
  ...
  echo -e "${BARRIER}   ╚╔════════════╝═════════════════════════╗ ${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [1]  Git          ${INFO}- Version control${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [2]  Curl         ${INFO}- HTTP requests${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [3]  Wget         ${INFO}- File downloads${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [4]  Bat          ${INFO}- 'cat' with syntax highlighting${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [5]  Htop         ${INFO}- Process viewer${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [6]  Fish         ${INFO}- Friendly shell${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [7]  OpenSSH      ${INFO}- SSH access${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [8]  Python       ${INFO}- Programming language${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [9]  Nmap         ${INFO}- Network scanning${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [10] Libcaca      ${INFO}- Fire magic effects${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [11] Speedtest-go ${INFO}- Internet speed-test${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [12] Cpufetch     ${INFO}- CPU information${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [13] Neofetch     ${INFO}- System information${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [14] Ranger       ${INFO}- Terminal file manager with VI keybindings${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [15] Nano         ${INFO}- Text editor${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [16] Sl           ${INFO}- Steam Locomotive (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [17] Ncdu         ${INFO}- NCurses disk usage analyzer${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [18] Neovim       ${INFO}- Best code editor${RST}"
  ...
  echo -e "${BARRIER}    ║${RST}${OPTION} [21] Croc         ${INFO}- File transferring tool ${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [22] Fzf          ${INFO}- File finder${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [23] Zoxide       ${INFO}- Smarter cd command (z替代品)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [24] Zsh          ${INFO}- Best shell ever${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [25] Duf          ${INFO}- Disk Usage/Free utility${RST}"
  ...
  echo -e "${BARRIER}    ║${RST}${OPTION} [28] Yazi         ${INFO}- Amazing filemanager for terminal${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [29] Lsd          ${INFO}- Another 'ls' alternative with icons/colors${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [30] Broot        ${INFO}- Navigate directories with overviews${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [31] Dust         ${INFO}- More intuitive version of du${RST}"
  ...
  echo -e "${BARRIER}   ╚╔═══════════╝═════════════════════════╗ ${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [01] Holehe          ${INFO}- Email OSINT scanner${RST}"
