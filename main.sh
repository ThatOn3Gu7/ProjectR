...
EOF
  ...
  echo -e "${BARRIER}   ╚╔════════════╝═════════════════════════╗ ${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [1]  Git          ${INFO}- Version control (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [2]  Curl         ${INFO}- HTTP requests (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [3]  Wget         ${INFO}- File downloads (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [4]  Bat          ${INFO}- 'cat' with syntax highlighting (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [5]  Htop         ${INFO}- Process viewer (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [6]  Fish         ${INFO}- Friendly shell (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [7]  OpenSSH      ${INFO}- SSH access (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [8]  Python       ${INFO}- Programming language (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [9]  Nmap         ${INFO}- Network scanning (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [10] Libcaca      ${INFO}- Fire magic effects (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [11] Speedtest-go ${INFO}- Internet speed-test (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [12] Cpufetch     ${INFO}- CPU information (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [13] Neofetch     ${INFO}- System information (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [14] Ranger       ${INFO}- Terminal file manager with VI keybindings (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [15] Nano         ${INFO}- Text editor (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [16] Sl           ${INFO}- Steam Locomotive (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [17] Ncdu         ${INFO}- NCurses disk usage analyzer (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [18] Neovim       ${INFO}- Best code editor (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [19] Cbonsai      ${INFO}- Japanise bonsai tree (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [20] Asciinema    ${INFO}- Terminal recording tool (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [21] Croc         ${INFO}- File transferring tool (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [22] Fzf          ${INFO}- File finder (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [23] Zoxide       ${INFO}- Smarter cd command (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [24] Zsh          ${INFO}- Best shell ever (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [25] Duf          ${INFO}- Disk Usage/Free utility (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [26] tty-clock    ${INFO}- A terminal clock (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [27] Pipes.sh     ${INFO}- A cool tool for (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [28] Yazi         ${INFO}- Amazing filemanager for terminal (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [29] Lsd          ${INFO}- An 'ls' alternative with icons/colors (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [30] Broot        ${INFO}- Navigate directories with overviews (Min)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [31] Dust         ${INFO}- More intuitive version of du (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [32] Procs        ${INFO}- Modern replacement for 'ps' (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [33] Tldr         ${INFO}- Simplified, community-driven man pages (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [34] Node.Js      ${INFO}- JavaScript Runtime environment (Dev)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [35] Gh           ${INFO}- A github-cli client (Dev)${RST}"
  echo -e "${BARRIER}    ╚══════════════════════════════════════╝ ${RST}"
  ...
  echo -e "${BARRIER}   ╚╔═══════════╝═════════════════════════╗ ${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [01] Holehe          ${INFO}- Email OSINT scanner (OSINT)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [02] Asciiquarium    ${INFO}- View of the sea (Fun)${RST}"
  echo -e "${BARRIER}    ║${RST}${OPTION} [03] Wttr            ${INFO}- A weather tool (Fun)${RST}"
  echo -e "${BARRIER}    ╚═════════════════════════════════════╝ ${RST}"
